...
<head>
    ...
    <title>Indigency Requests / BagKat</title>
    <link rel="stylesheet" href="../css/admin_indigency.css">
</head>
...
<body>
    ...
    <div class="main-content">
        <div class="main">
            <h1>CERTIFICATION APPROVAL: BARANGAY INDIGENCY</h1>
        </div>
                    ...
                    <tbody>
                        ...
                        if ($_SERVER["REQUEST_METHOD"] == "POST") {
                            if (isset($_POST['approve_request'])) {
                                ...
                                $sql = "UPDATE barangay_indigency SET status = 'Approved' WHERE id = $id";
                                if ($conn->query($sql) === TRUE) {
                                    echo "<script>alert('Request with ID $id has been approved!'); window.location = 'admin_indigency.php';</script>";
                                } else {
                                    echo "Error updating record: " . $conn->error;
                                }
                            } elseif (isset($_POST['decline_request'])) {
                                $id = $_POST['decline_request'];
                                $sql = "UPDATE barangay_indigency SET status = 'Declined' WHERE id = $id";
                                if ($conn->query($sql) === TRUE) {
                                    echo "<script>alert('Request with ID $id has been declined!'); window.location = 'admin_indigency.php';</script>";
                                } else {
                                    echo "Error updating record: " . $conn->error;
                                }
                            }
                        ...
                        $sql = "SELECT * FROM barangay_indigency";
